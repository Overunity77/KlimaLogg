...
//	if (dev->ctrl_read_urb)
//		usb_kill_urb(dev->ctrl_read_urb);
//
//	if (dev->ctrl_read_urb2)
//		usb_kill_urb(dev->ctrl_read_urb2);
}
...
//	if (dev->ctrl_read_urb)
//		usb_free_urb(dev->ctrl_read_urb);
//	if (dev->ctrl_read_urb2)
//		usb_free_urb(dev->ctrl_read_urb2);

	kfree(dev->int_in_buffer);
	kfree(dev->ctrl_buffer);
	kfree(dev->ctrl_dr);
//	kfree(dev->ctrl_read_buffer);
//	kfree(dev->ctrl_read_buffer2);
//	kfree(dev->ctrl_read_dr);
//	kfree(dev->ctrl_read_dr2);
	kfree(dev);
}
...
{
	struct usb_kl *dev = urb->context;
	DBG_INFO("urb->status: %d, urb->transfer_flags: 0x%x, urb->transfer_dma: 0x%x, urb->error_count: %d", urb->status, urb->transfer_flags, (unsigned int)urb->transfer_dma, urb->error_count);
	kl_debug_data(__FUNCTION__, urb->actual_length, urb->transfer_buffer);
	dev->ctrl_read_transfer_count = urb->actual_length;
}
...
static void kl_ctrl_read_callback(struct urb *urb)
{
	...
	dev->ctrl_read_dr2->bRequestType = 0xa1;
	dev->ctrl_read_dr2->bRequest     = 0x01;
	dev->ctrl_read_dr2->wValue       = cpu_to_le16(0x03dc);	/* convert to little-endian */
	dev->ctrl_read_dr2->wIndex       = cpu_to_le16(0);
	dev->ctrl_read_dr2->wLength      = cpu_to_le16(KL_CTRL_READ_BUFFER2_SIZE);

	usb_fill_control_urb(dev->ctrl_read_urb2, dev->udev,
			usb_sndctrlpipe(dev->udev, 0),
			(unsigned char *)dev->ctrl_read_dr2,
			dev->ctrl_read_buffer2,
			KL_CTRL_READ_BUFFER2_SIZE,
			kl_ctrl_read_callback2,
			dev);


	DBG_INFO("going to submit control urb");

	ret = usb_submit_urb(dev->ctrl_read_urb2, GFP_ATOMIC);
	if (ret) {
		DBG_ERR("submitting get_report control URB failed (%d)",
				ret);
	}
}
...
static void kl_int_in_callback(struct urb *urb)
{
	...
	DBG_INFO("urb->status = %d (-2 = -ENOENT: The urb was stopped by a call to usb_kill_urb)", urb->status);
...
static ssize_t kl_write(struct file *file, const char __user *user_buf, size_t count, loff_t *ppos)
{
	...
	dev->ctrl_read_transfer_count = 0;
	...
	dev->ctrl_read_urb = usb_alloc_urb(0, GFP_KERNEL);
	if (! dev->ctrl_read_urb) {
	...
	dev->ctrl_read_urb2 = usb_alloc_urb(0, GFP_KERNEL);
	if (! dev->ctrl_read_urb2) {
	...
	dev->ctrl_read_buffer = kzalloc(KL_CTRL_READ_BUFFER_SIZE, GFP_KERNEL);
	if (! dev->ctrl_read_buffer) {
		DBG_ERR("could not allocate ctrl_read_buffer");
		retval = -ENOMEM;
		goto error;
	}
	dev->ctrl_read_buffer2 = kzalloc(KL_CTRL_READ_BUFFER2_SIZE, GFP_KERNEL);
	if (! dev->ctrl_read_buffer2) {
		DBG_ERR("could not allocate ctrl_read_buffer2");
		retval = -ENOMEM;
		goto error;
	}

	dev->ctrl_read_dr = kmalloc(sizeof(struct usb_ctrlrequest), GFP_KERNEL);
	if (! dev->ctrl_read_dr) {
		DBG_ERR("could not allocate usb_ctrlrequest");
		retval = -ENOMEM;
		goto error;
	}
	dev->ctrl_read_dr2 = kmalloc(sizeof(struct usb_ctrlrequest), GFP_KERNEL);
	if (! dev->ctrl_read_dr2) {
		DBG_ERR("could not allocate usb_ctrlrequest2");
		retval = -ENOMEM;
		goto error;
	}

	dev->ctrl_read_dr->bRequestType = KL_CTRL_READ_REQUEST_TYPE;
	dev->ctrl_read_dr->bRequest = KL_CTRL_SET_REPORT_REQUEST;
	dev->ctrl_read_dr->wValue = cpu_to_le16(0x03dd);	/* convert to little-endian */
	dev->ctrl_read_dr->wIndex = cpu_to_le16(0);
	dev->ctrl_read_dr->wLength = cpu_to_le16(KL_CTRL_READ_BUFFER_SIZE);

	usb_fill_control_urb(dev->ctrl_read_urb, dev->udev,
			usb_sndctrlpipe(dev->udev, 0),
			(unsigned char *)dev->ctrl_read_dr,
			dev->ctrl_read_buffer,
			KL_CTRL_READ_BUFFER_SIZE,
			kl_ctrl_read_callback,
			dev);

	dev->ctrl_read_buffer[0]  = 0xdd;
	dev->ctrl_read_buffer[1]  = 0x0a;
	dev->ctrl_read_buffer[2]  = 0x01;
	dev->ctrl_read_buffer[3]  = 0xF5;
	dev->ctrl_read_buffer[4]  = dev->ctrl_read_buffer[5]  = dev->ctrl_read_buffer[6]  = dev->ctrl_read_buffer[7]  = 0xcc;
	dev->ctrl_read_buffer[8]  = dev->ctrl_read_buffer[9]  = dev->ctrl_read_buffer[10] = dev->ctrl_read_buffer[11] = 0xcc;
	dev->ctrl_read_buffer[12] = dev->ctrl_read_buffer[13] = dev->ctrl_read_buffer[14] = dev->ctrl_read_buffer[15] = 0xcc;

	kl_debug_data(__FUNCTION__, KL_CTRL_READ_BUFFER_SIZE, dev->ctrl_read_buffer);

	DBG_INFO("going to submit control urb");

	ret = usb_submit_urb(dev->ctrl_read_urb, GFP_ATOMIC);
	if (ret) {
		DBG_ERR("submitting set_report control URB failed (%d)",
				ret);
	}
		...
		loff_t *ppos)
{
	...
	DBG_INFO("read transfer count: %u", dev->ctrl_read_transfer_count);

	retval = dev->ctrl_read_transfer_count;

	if (*ppos == retval) return 0;

	if (copy_to_user(buffer, dev->ctrl_read_buffer2, retval)) {
		retval = -EFAULT;
	}
...
static int kl_probe(struct usb_interface *interface, const struct usb_device_id *id)
{
	...
	DBG_INFO("interrupt endpoint size: %d", int_end_size);
